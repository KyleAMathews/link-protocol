...
export const loader = async () => {
  ...
  picked.forEach(message => {
    message.links.forEach(link => links.push({link, reactions: message.reactions}))
  })
  return json({
  links: sortBy(links, (link) => sumBy(link.reactions, (reaction) => reaction.count)).reverse()
  });
};
...
export default function Index() {
  ...
  return (
    ...
    <h1 className="text-2xl mb-2">Links</h1>
      {data.links.map(link => {
        return (
        <div>- {link.link} {link.reactions?.map(reaction => <span>{reaction.name} {reaction.count}{`, `}</span>)}</div>
        )
      })}
      </div>
    </main>
